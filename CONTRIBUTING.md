...
```
pubspec.yaml package.json package-lock.json README.md CHANGELOG.md showcase/pubspec.yaml showcase/lib/app_info/flowin_app_info_service.dart
```
...
This has bitten twice in a sibling package, both times a directory added for a
good reason and never listed: `doc/api/` (generated docs) and `tool/` (release
scripts). Scan the tree for anything that is not `lib/`, `example/`, the README,
the changelog, the license and the pubspec.
