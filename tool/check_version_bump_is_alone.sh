...
#
# This package carries its version in five places rather than three: the
# showcase app pins it too, and its version label reads a Dart constant. See
# CONTRIBUTING's table. A release PR legitimately touches all of them, so
# leaving them out would fail every real bump.
ALLOWED="pubspec.yaml package.json package-lock.json README.md CHANGELOG.md showcase/pubspec.yaml showcase/lib/app_info/flowin_app_info_service.dart"
