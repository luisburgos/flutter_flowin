...
new_repo() {
  ...
  printf 'name: flutter_flowin\nversion: 0.3.0\ndescription: kit\n' > "$dir/pubspec.yaml"
  printf '{\n  "name": "flutter_flowin",\n  "version": "0.3.0"\n}\n' > "$dir/package.json"
  printf 'dependencies: flutter_flowin ^0.3.0\n' > "$dir/README.md"
  printf '# Changelog\n' > "$dir/CHANGELOG.md"
  printf 'code\n' > "$dir/lib.dart"
  mkdir -p "$dir/showcase/lib/app_info"
  printf 'name: showcase\nversion: 0.3.0\n' > "$dir/showcase/pubspec.yaml"
  printf "const flowinVersion = '0.3.0';\n" > "$dir/showcase/lib/app_info/flowin_app_info_service.dart"
  git -C "$dir" add -A
...
# --- this package versions its showcase too -----------------------------------
#
# Five version places rather than three, so a real bump touches the showcase's
# pubspec and its version constant. A default allowlist would reject every
# legitimate release PR here. Shaped after the real 0.3.0 bump (aaa00cb).

d=$(new_repo showcase_files)
sed -i.bak 's/^version: 0.3.0/version: 0.4.0/' "$d/pubspec.yaml"
sed -i.bak 's/"version": "0.3.0"/"version": "0.4.0"/' "$d/package.json"
sed -i.bak 's/^version: 0.3.0/version: 0.4.0/' "$d/showcase/pubspec.yaml"
sed -i.bak "s/0.3.0/0.4.0/" "$d/showcase/lib/app_info/flowin_app_info_service.dart"
sed -i.bak 's/\^0.3.0/^0.4.0/' "$d/README.md"
rm -f "$d"/*.bak "$d"/showcase/*.bak "$d"/showcase/lib/app_info/*.bak
commit_all "$d" "chore(release): bump to 0.4.0"
assert "a bump carrying the showcase version files is allowed" 0 "$d"

d=$(new_repo showcase_piggyback)
sed -i.bak 's/^version: 0.3.0/version: 0.4.0/' "$d/pubspec.yaml"
rm -f "$d"/*.bak
printf 'code\nfeature\n' > "$d/lib.dart"
commit_all "$d" "feat: piggyback alongside a showcase-shaped bump"
assert "source changes still block a bump here" 1 "$d"
